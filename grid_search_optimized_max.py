...
def generate_prior_grid_n5(n_balls=50,
                        p0_range=(0.0, 0.1),
                        p12_range=(0.0, 0.1),     # p1 = p2 (tied to one axis)
                        p3_range=(0.6, 0.7),
                        p4_range=(0.2, 0.3)):
    step = 1.0 / n_balls
    ...
    def axis(lo, hi):
        ...
        vals = [round(lo + i * step, 4) for i in range(k + 1)]
        return [v for v in vals if v > 0.0]       # keep strictly positive
 
    p0_vals = axis(*p0_range)
    p12_vals = axis(*p12_range)
    p3_vals = axis(*p3_range)
    ...
    for p0 in p0_vals:
        for p12 in p12_vals:
            for p3 in p3_vals:
                p4 = 1 - p0 - 2*p12 - p3
                if p4 > 0.2:
                    grids.append(np.array([p0, p12, p12, p3, p4]).round(4))
    return grids
...
if __name__ == "__main__":
    ...
    # prior_grid = generate_prior_grid_n5(n_balls=1000)
    prior_grid = [np.array([1/N_THRESHOLDS for _ in range(N_THRESHOLDS)])]
    partition_base = [[i] for i in range(N_THRESHOLDS)]

    # ---- sweep axes ---------------------------------------------------------
    c_values = [1.0]
    tt_values = [0.01, 0.05, 0.1, 0.15]
    ...
    filepath = f"grid_search_n{N_THRESHOLDS}_c_tt_sweep_heuristic_refined.pkl"
